...
export const testRun = {
  ...
  specs: [
    ...
    { name: "a linked row is not hidden under the sticky header", ms: "882ms" },
    ...
    { name: "the private pages also carry noindex", ms: "1.20s" },
    { name: "the public pages are left indexable", ms: "660ms" },
    {
      name: "an invalid field is marked, and its message is tied to it",
      ms: "1.69s",
    },
    { name: "optional fields are not marked invalid or required", ms: "1.90s" },
    {
      name: "the required fields say so in the accessibility tree",
      ms: "630ms",
    },
    { name: "visitor reaches the quote form", ms: "2.27s" },
    { name: "a bad email address is caught", ms: "2.12s" },
    { name: "the quote form submits", ms: "2.16s" },
    { name: "the lead is stored, not lost", ms: "2.36s" },
    { name: "a lead from the QR page records where it came from", ms: "801ms" },
    { name: "spam is turned away", ms: "37ms" },
    { name: "a lead is never falsely reported as emailed", ms: "42ms" },
    {
      name: "an unanswered lead past the threshold is picked up by the sweep",
      ms: "77ms",
    },
    { name: "a lead inside the threshold is left alone", ms: "48ms" },
    { name: "a lead already marked handled is never chased", ms: "22ms" },
    { name: "a lead already reminded is not reminded again", ms: "22ms" },
    { name: "nothing is sent outside the sending window", ms: "29ms" },
    { name: "the sweep refuses a caller without the secret", ms: "18ms" },
    {
    ...
    {
      name: "nothing is clipped at 320px, least of all the phone number",
      ms: "1.49s",
    },
  ],
} as const;
